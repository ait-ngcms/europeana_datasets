package eu.europeana.api.client.datasets.eusounds.content;
...
public class ContentAccessorTest extends EuClientDatasetUtil {
	...
	public void saveSoundContentMap() throws IOException, EuropeanaApiProblem {
		...
		if(!toFile.exists()){
			...
			//Map<String, String> contentMap = ca.getContentMap(CommonMetadata.EDM_FIELD_IS_SHOWN_BY, -1, 5, MetadataAccessor.ERROR_POLICY_CONTINUE);
			...
			Assert.fail("The provided content file already exists on the given location. Please delete the file and run the test again!" + toFile.getAbsolutePath());
			//fail("");
		}
	}
